// src/pages/PDFEditor/PDFEditor.js - Fixed Canvas Render Management
...
function PDFEditor({ pdf, job, onClose, onSave }) {
  ...
  const loadPDF = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3005/api/job/${job.id}/attachment/${pdf.serviceTitanId}/download`
      );
  ...
  const renderPage = async () => {
    ...
    try {
      ...
      // Clear reference after successful render
      renderTaskRef.current = null;
    } catch (error) {
  ...
  // Page navigation with render management
  const changePage = async (newPage) => {
    if (newPage < 1 || newPage > totalPages || newPage === currentPage) return;

    // Cancel current render if in progress
    if (renderTaskRef.current) {
      await renderTaskRef.current.cancel();
      renderTaskRef.current = null;
    }
  ...
  // Scale change with render management
  ...
  const handleCanvasMouseDown = useCallback(
    (e) => {
      if (isRendering) return; // Don't interact while rendering
    ...
    [objects, currentPage, scale, isRendering]
  );
  ...
  const handleMouseMove = useCallback(
    (e) => {
      if (!dragState || isRendering) return;
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.helpBar}>
        Click anywhere to add text • Double-click text to edit • Drag handles to
        move/resize
      </div>
    </div>
  );
...
function SignatureCanvas({ object, scale, onUpdate, disabled }) {
  const canvasRef = useRef(null);
