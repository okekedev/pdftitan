...
function usePDFEditor(pdf, job) {
  ...
  /**
   * Load PDF document with optimized error handling and fallback approaches
   */
  const loadPDF = useCallback(async () => {
    ...
    try {
      ...
      console.log('🔍 PDF object properties:', {
        id: pdf.id,
        ...
        hasUrl: !!pdf.url,
        hasDownloadUrl: !!pdf.downloadUrl,
        allProperties: Object.keys(pdf)
      });
      
      // Try to import PDF.js with fallback
      let pdfjs;
      try {
        pdfjs = await import('pdfjs-dist');
      } catch (importError) {
        // If pdfjs-dist is not available, try loading from CDN
        console.warn('pdfjs-dist not found locally, trying CDN fallback');
      ...
      if (pdf.dataUrl && pdf.dataUrl.startsWith('data:')) {
        ...
        console.log('📄 Using data URL source');
        ...
        // Handle ServiceTitan attachments via our backend API
        const downloadUrl = `/api/job/${jobId}/attachment/${pdf.serviceTitanId}/download`;
        console.log('🔗 Fetching PDF from ServiceTitan via backend:', downloadUrl);
        ...
        try {
          ...
          const arrayBuffer = await response.arrayBuffer();
          pdfSource = { data: arrayBuffer };
          console.log('✅ PDF fetched from ServiceTitan, size:', arrayBuffer.byteLength, 'bytes');
        } catch (fetchError) {
          console.error('❌ Failed to fetch PDF from ServiceTitan:', fetchError);
          throw new Error(`Failed to fetch PDF from ServiceTitan: ${fetchError.message}`);
        }
      } else {
        console.error('❌ PDF object:', pdf);
        console.error('❌ Missing properties - PDF needs either:');
        console.error('   - dataUrl (data:application/pdf;base64,...)');
        console.error('   - url or downloadUrl (http://...)');
        console.error('   - serviceTitanId + jobId for ServiceTitan attachments');
        throw new Error('No valid PDF source found. Check PDF object properties.');
      }
      ...
      let errorMessage = error.message || 'Failed to load PDF';
      
      if (errorMessage.includes('pdfjs-dist')) {
        errorMessage = 'PDF.js library is not installed. Please run: npm install pdfjs-dist@3.11.174';
      } else if (errorMessage.includes('Failed to fetch')) {
        errorMessage = 'Cannot access PDF file. Please check the file URL and network connection.';
